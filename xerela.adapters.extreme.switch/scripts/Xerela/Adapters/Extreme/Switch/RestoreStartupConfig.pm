package Xerela::Adapters::Extreme::Switch::RestoreStartupConfig;
...
sub restore_via_tftp
{
	...
	my @responses = ();
	push(@responses, Xerela::Response->new('reboot the system\? \(Y\/N\)', \&_finish));
	push(@responses, Xerela::Response->new('(?i:ERROR)', \&restore_via_tftp));
