package Xerela::Adapters::Nortel::BayRS::Parsers;

use strict;
use warnings;
use Xerela::Addressing::Subnet;
use Xerela::Adapters::Utils qw(mask_to_bits strip_mac seconds_since_epoch get_mask get_port_number trim get_interface_type parseCIDR bin2dec);
...
sub parse_interfaces
{
	...
	while ( $in->{interfaces} =~ /^(.+)$/mig )
	{
			...
			&& defined $interface->{name} )
		{
			...
			my $subnet = new Xerela::Addressing::Subnet( $1, mask_to_bits($2) );
...
Xerela::Adapters::Nortel::BayRS::Parsers

=head1 SYNOPSIS

	use Xerela::Adapters::Nortel::BayRS::Parsers qw(parse_routing create_config parse_local_accounts parse_chassis parse_filters parse_snmp parse_system parse_interfaces parse_static_routes parse_vlans parse_stp);
