package Xerela::Adapters::ThreeCom::Switch4400::Restore;
...
use Xerela::Adapters::ThreeCom::Switch4400::AutoLogin;
use Xerela::Adapters::ThreeCom::Switch4400::Disconnect
	qw(disconnect);
use Xerela::Logger;
...
sub _connect
{
	...
	my $device_prompt_regex = Xerela::Adapters::ThreeCom::Switch4400::AutoLogin::execute( $cli_protocol, $connection_path );
...
sub restore_via_tftp
{
	...
	my @responses = ();
	push( @responses, Xerela::Response->new( 'TFTP [Ss]erver IP [Aa]ddress',	\&_send_tftp_ip ) );
	push( @responses, Xerela::Response->new( '[F]ile [Nn]ame',					\&_send_source_filename ) );
	push( @responses, Xerela::Response->new( '[Ee]nter [Uu]ser [Nn]otes',		\&_send_user_note ) );
	push( @responses, Xerela::Response->new( 'Do you wish to continue',			\&_send_yes ) );
	push( @responses, Xerela::Response->new( '[Ee]rror: [Rr]estore [Ff]ailed',	undef, 				$TFTP_ERROR ) );
...
sub _send_user_note
{
	...
	my @responses = ();
	push( @responses, Xerela::Response->new( 'Do you wish to continue',										\&_send_yes ) );
	push( @responses, Xerela::Response->new( '[Rr]estore [Oo]f [Ss]ystem [Cc]onfiguration [Ss]uccessful', 	\&_finish ) );
	push( @responses, Xerela::Response->new( '[Ee]rror: [Rr]estore [Ff]ailed',								undef, $TFTP_ERROR ) );
