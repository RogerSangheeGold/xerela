...
eval {
	...
	Xerela::SafeCopy::safe_copy( $fullFilePath, $tftpServer->get_root_dir() . '/' . $shortFileName );
	...
	my $prompt      = Xerela::Adapters::Cisco::IOS::AutoLogin::execute( $cliProtocol, $connectionPath );
...
Xerela::SafeCopy::safe_delete( $tftpServer->get_root_dir() . '/' . $shortFileName );
