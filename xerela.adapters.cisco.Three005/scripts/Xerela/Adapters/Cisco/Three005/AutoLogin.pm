package Xerela::Adapters::Cisco::Three005::AutoLogin;
...
sub _initial_connection
{
	...
	push(@responses, Xerela::Response->new('->', \&_get_prompt));
...
sub _send_password
{
	...
	# Specify the responses to check for
	my @responses = ();
	push(@responses, Xerela::Response->new('denied', undef, $INVALID_CREDENTIALS));
	push(@responses, Xerela::Response->new('nvalid', undef, $INVALID_CREDENTIALS));
	push(@responses, Xerela::Response->new('->', \&_get_prompt));
...
Xerela::Adapters::Cisco::Three005::AutoLogin - An example autologin module

=head1 SYNOPSIS

    use Xerela::Adapters::Cisco::Three005;
	my $cli_protocol = Xerela::CLIProtocolFactory::create( $connection_path );
	my $prompt_regex = Xerela::Adapters::Cisco::Three005::execute( $cli_protocol, $connection_path );
