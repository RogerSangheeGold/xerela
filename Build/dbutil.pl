...
sub reset_mysql()
{
	...
	print WRITEME "DROP DATABASE IF EXISTS xerela; CREATE DATABASE xerela DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;";
	close(WRITEME);
}
...
sub reset_pgsql()
{
	...
	$cmd .= '-c ';
	system( $cmd . '"DROP DATABASE xerela"' );
	system( $cmd . '"CREATE DATABASE xerela ENCODING=\'UTF8\'"' );
}
...
sub run_sql_derby
{
	...
	if ( $Config::Config{'osname'} =~ /(?<!dar)win/i )
	{
		$java_opts .= " -Dij.database=jdbc:derby:xerela;create=true;user=$user;password=$password";
	}
	else
	{
		$java_opts .= " -Dij.database=jdbc:derby:xerela\\;create=true\\;user=$user\\;password=$password";
	...
	# my $pid = open(WRITEME, "| $cmd");
	# print WRITEME "DROP DATABASE xerela; CREATE DATABASE xerela;";
	# close(WRITEME);
}
...
sub print_usage
{
    print "Xerela Database Utility - ^VERSION^\n\n";
	...
	print "  --dir  the Xerela installation directory (optional)\n";

	print "\nActions:\n";
	print "   reset  reset Xerela to it's original state\n";
	print "\n";
