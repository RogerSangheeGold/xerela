package Xerela::Adapters::Foundry::FastIron::Parsers;

use strict;
use warnings;
use Xerela::Addressing::Subnet;
use Xerela::Adapters::Utils qw(mask_to_bits seconds_since_epoch get_mask get_port_number trim get_interface_type strip_mac parseCIDR get_crep);
	...
	while ( $in->{interfaces} =~ /^(.+)$/mig )
	{
			...
			&& defined $interface->{name} )
		{
			...
			if ( $if_line =~ /\bInternet\s+address\s+is\s+($CIPM_RE)\/(\d+),/i )
			{
				...
				my $subnet = new Xerela::Addressing::Subnet( $1, $2 );
				push( @{ $subnets->{$interface->{name}} }, $subnet );
			}
		}
...
Xerela::Adapters::Foundry::FastIron::Parsers
...
Foundry device responses and print out Xerela model elements.
...
Uses the Foundry startup and running configs to put together a Xerela
