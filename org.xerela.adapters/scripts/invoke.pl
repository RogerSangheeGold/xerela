...
# Pull off the adapter ID/package name (for example: "Xerela::Adapters::Cisco::IOS")
...
invoke.pl - Invokes an adapter operation using the Xerela Perl Adapter SDK.

=head1 SYNOPSIS

    invoke.pl <adapter_package> <operation_name> [-i <input-file>]|[-x <contents>]
    invoke.pl Xerela::Adapters::Cisco::IOS backup operationInput.xml

=head1 DESCRIPTION

The C<invoke.pl> Perl script invokes an operation for a specified adapter using the Xerela Perl Adapter SDK.  This achieved
...
The C<invoke.pl> Perl script is considered to be the main entry point for any operation when invoked by Xerela.  Once
the package name of the Xerela adapter module is determined, the C<invoke.pl> Perl script will execute the subroutine that
...
The name/id of the Xerela adapter that contains the operation that will be executed.  The name/id of a Xerela adapter
is the full package name of the Perl module that implements the adapter.  For example, the Xerela adapter that implements
support for the Cisco IOS device family has the adapter name/id of C<Xerela::Adapters::Cisco::IOS>.  The reason the package
name is used as the name/id of a Xerela adapter is to easily call any operations that are implemented on the adapter module
itself.  It is expected that the adapter ID/name match both the name of the Xerela adapter module and that "adapterId" element defined within
...
adapter module that can be invoked; the specified operation B<MUST> be implemented on the Xerela adapter module
