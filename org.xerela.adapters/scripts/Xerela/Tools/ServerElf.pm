package Xerela::Tools::ServerElf;
...
sub get_zed
{
	...
	return get_configuration($ipAddress, $managedNetwork, '/Xerela-Element-Document');
}
...
Xerela::Tools::ServerElf

=head1 SYNOPSIS

    use Xerela::Tools::ServerElf qw(get_zed);
	my $zed = get_zed($ipAddress, $managedNetwork);
	
    use Xerela::Tools::ServerElf qw(update_credential);
...
from the Xerela server, i.e. it doesn't need to know how to login.
...
Retrieves the latest XerelaElementDocument, or ZED, for the provided device.
...
slash, '/'.  For example:
	...
	my $runningConfig = Xerela::Tools::ServerElf::get_configuration('10.100.4.8', 'Default', '/running-config');
...
Utility method to update existing credentil definitions, one key at a time.
	...
	$credentialKey - the credential key must be well known to the Xerela server. e.g. 'enablePassword'.
...
For example, if you would like to update the password for a device, but only for the username 'ryan', then you would do the following:
	...
	use Xerela::Tools::ServerElf qw(update_dependent_credential);
