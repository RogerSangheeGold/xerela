package Xerela::CLIProtocolFactory;
...
sub create
{
	...
	if (@_ == 1)
	{
		...
		if ($arg_ref_name eq "Xerela::ConnectionPath")
		{
			...
			# Use the "create_from_connection_path" sub-routine to create a Xerela::CLIProtocol object
		...
		# If the argument is not a Xerela::ConnectionPath, then assume it is a protocol name
		else
		{
			$protocol_name = $arg;
		}
...
sub create_from_connection_path
{
	...
	# Create an undef reference to a Xerela::CLIProtocol object which will hopefully be set
...
Xerela::CLIProtocolFactory - Factory for creating Xerela::CLIProtocol objects.

=head1 SYNOPSIS

	use Xerela::CLIProtocolFactory;

	my $cli_protocol = Xerela::CLIProtocolFactory::create("Telnet");

	my $cli_protocol = Xerela::CLIProtocolFactory::create("SSH", 10.10.10.10, 22);

	my $cli_protocol = Xerela::CLIProtocolFactory::create($connection_path);

=head1 DESCRIPTION

C<Xerela::CLIProtocolFactory> is a simple factory to ease the creation of instances of C<Xerela::CLIProtocol>
...
as a C<Xerela::CLIProtocol> object should I<NEVER> be created explicitly; only sub-classes/implementations
of the C<Xerela::CLIProtocol> interface should be created.
...
Creates an instance of the subclass of C<Xerela::CLIProtocol> that is the implementation of the specified
CLI protocol.  If the specified CLI protocol is not support or implemented, an error will be thrown.

There are two ways to create a C<Xerela::CLIProtocol> object using this factory:
...
stored on the newly created C<Xerela::CLIProtocol> object for potential later use, such as using them when invoking
the C<Xerela::CLIProtocol::connect($ip_address, $port, $username, $password)> method.

2) Specify a C<Xerela::ConnectionPath> object that represents a connection to a device using a CLI protocol.
