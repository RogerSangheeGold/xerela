package Xerela::TransferProtocolFactory;
...
sub create
{
	...
	# If only one argument was specified, check to see if it is a Xerela::ConnectionPath instance or not
	if (@_ == 1)
	{
		my $arg = shift;
		my $arg_ref_name = ref($arg);
		if ($arg_ref_name eq "Xerela::ConnectionPath")
		{
			my $connection_path = $arg;
	
			# Identify the Xerela::NewConnection path object
			my $connection_path = $arg;
			
			# Use the "create_from_connection_path" sub-routine to create a Xerela::TransferProtocol object
		...
		# If the argument is not Xerela::ConnectionPath instance, then assume it is a protocol name
		else
		{
			$protocol_name = $arg;
		}
	...
	# Return the newly created Xerela::TransferProtocol object
...
sub create_from_connection_path
{
	...
	# Create an undef reference to a Xerela::TransferProtocol object which will hopefully be set
	my $xfer_protocol = undef;
	
	if ( defined($connection_path) )
	{
		# Check to see if Telnet or SSH is a specified protocol on the Xerela::ConnectionPath object
	...
	# Return the hopefully populated Xerela::CLIProtocol object
	return $xfer_protocol;
}
...
Xerela::TransferProtocolFactory - Factory for creating Xerela::TransferProtocol objects.

=head1 SYNOPSIS

	use Xerela::TransferProtocolFactory;

	my $xfer_protocol = Xerela::TransferProtocolFactory::create("TFTP");
	my $xfer_protocol = Xerela::TransferProtocolFactory::create("SCP", 10.10.10.10);
	my $xfer_protocol = Xerela::TransferProtocolFactory::create("FTP", 10.10.10.10, 21);
	my $xfer_protocol = Xerela::TransferProtocolFactory::create($connection_path);

=head1 DESCRIPTION

C<Xerela::TransferProtocolFactory> is a simple factory to ease the creation of instances of C<Xerela::TransferProtocol>
...
as a C<Xerela::TransferProtocol> object should I<NEVER> be created explicitly; only sub-classes/implementations
of the C<Xerela::TransferProtocol> interface should be created.
...
Creates an instance of the subclass of C<Xerela::TransferProtocol> that is the implementation of the specified
file transfer protocol.  If the specified file transfer protocol is not support or implemented, an error will be thrown.

There are two ways to create a C<Xerela::TransferProtocol> object using this factory:
...
stored on the newly created C<Xerela::TransferProtocol> object for potential later use, such as using them when invoking
the C<connect($ip_address, $port, $username, $password)> method.

2) Specify a C<Xerela::ConnectionPath> object that represents a connection to a device, as well as information needed for
