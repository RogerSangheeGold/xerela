...
sub from_xml
{
	my $in_value = shift or $LOGGER->fatal( "ERROR - No XML specified to convert into a Xerela::Credentials object!" );
	...
	return Xerela::Credentials->new(%{$credential_xml_hash_ref});
}
...
Xerela::Credentials - A simple hash to store the various credential types that are supported by Xerela.

=head1 SYNOPSIS

	use Xerela::Credentials;

	my $credentials = Xerela::Credentials->new(
...
The C<Xerela::Credentials> module defines a simple hash to store the various credential types that are supported by
Xerela.  Users of an instance C<Xerela::Credentials> module can reference any entry in the hash that represents a type of
credential to be used by referencing the credential type as the key on the C<Xerela::Credentials> module hash itself.
...
Creates a new instance of the C<Xerela::Credentials> module.  One or more credential types can be specified as arguments
...
Prints out the contents of this instance of the C<Xerela::Credentials> module a "key => value" style.
...
Creates a new instance of the C<Xerela::Credentials> module by parsing an input XML string that represents an XML element
