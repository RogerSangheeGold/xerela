...
sub sub_model_test
{
	...
	my $printer = Xerela::Model::XmlPrint->new( \*TMPFILE );
...
sub core_model_test
{
	...
	my $in  = $self->{_responses};
	open( TMPFILE, ">$xml" ) || die;
	my $printer = Xerela::Model::XmlPrint->new( \*TMPFILE, "core" );
	$printer->open_model();
	...
	$printer->close_model();
	close(TMPFILE);
	my $result = _validate_xml( $xml, "../../../org.xerela.adapters/schema/model/xerela-core.xsd" );
	ok( $result, $testLabel );
	return $result;
}
...
sub _inject_namespace
{
	...
	# first element.  This is used so that portions of the Xerela
...
TestElf - Used for testing adapter parsing code against the Xerela model (ZED).
...
complete the xerela-core model.
