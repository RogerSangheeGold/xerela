...
sub from_xml
{
	my $inValue = shift or $LOGGER->fatal("ERROR - No XML specified to convert into a Xerela::RestoreFile object!");
	my ($path) = $inValue =~ /path=['"](.+?)['"]\/?>/; 
	return new Xerela::Filestore->new($path);
}
...
Xerela::Filestore - A location where image files shouldl be saved to.  For use in the ospull operation.

=head1 SYNOPSIS

	This module is only created by the C<Xerela::Typer> module.
