package Xerela::ConnectionPath::Protocol;
...
sub new
{
	...
	# Turn $this into a Xerela::ConnectionPath::Protocol object
...
sub from_xml
{
	my $in_value = shift or $LOGGER->fatal( "ERROR - No XML specified to convert into a Xerela::ConnectionPath::Protocol object!" );
	...
	# Create an array that can store all of the created Xerela::ConnectionPath::Protocol objects
	my @protocols_array = ();

	# Convert all of the "protocol" instances into Xerela::ConnectionPath::Protocol objects
	foreach my $curr_protocol_name ( keys(%$protocols_xml_hash_ref) )
	{
		...
		# Use the updated hash as a parameter to create a new Xerela::ConnectionPath::Protocol object
		my $protocol_obj = Xerela::ConnectionPath::Protocol->new($current_protocol_hash);

		# Add the newly created Xerela::ConnectionPath::Protocol object to our array of Xerela::ConnectionPath::Protocol objects.
		push( @protocols_array, $protocol_obj );
	}

	# Return a reference to the array of Xerela::ConnectionPath::Protocol objects
...
Xerela::ConnectionPath::Protocol - Stores information regarding a network protocol

=head1 SYNOPSIS

    use Xerela::ConnectionPath::Protocol;
    my $protocols_array_ref = Xerela::ConnectionPath::Protocol->new($input_xml_doc);
	my $telnet_protocol = Xerela::ConnectionPath::Protocol->new(-name => "Telnet", -port => "23", -properties => { timeout => "30"});
...
The C<Xerela::ConnectionPath::Protocol> module provides a simple way to store and retreive metadata/information about a
network protocol.  The only information that can be stored is the name of the protocol, the port that the protocol uses, and
a hash of property values.  These property values can be set to anything and users of a C<Xerela::ConnectionPath::Protocol>
...
Creates an array of new instances of the C<Xerela::ConnectionPath::Protocol> module by parsing an input XML string that represents
...
Creates a new C<Xerela::ConnectionPath::Protocol> instance.  If no arguments are specified, then all of the internal
