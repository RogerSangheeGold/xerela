package Xerela::ConnectionPath::FileServer;
...
sub new
{
	...
	# Turn $this into a Xerela::ConnectionPath::FileServer object
...
sub from_xml
{
	my $in_value = shift or $LOGGER->fatal( "ERROR - No XML specified to convert into a Xerela::ConnectionPath::FileServer object!" );
	...
	# Create an array that can store all of the created Xerela::ConnectionPath::FileServer objects
	my @file_servers_array = ();

	# Convert all of the "fileServer" instances into Xerela::ConnectionPath::FileServer objects
	...
	{
		my $file_server_obj = Xerela::ConnectionPath::FileServer->new( $curr_file_server_xml_hash_ref);
		push( @file_servers_array, $file_server_obj );
	}

	# Return a reference to the array of Xerela::ConnectionPath::FileServer objects
...
Xerela::ConnectionPath::FileServer - Stores information regarding an external file server.

=head1 SYNOPSIS

    use Xerela::ConnectionPath::FileServer;
	my $tftp_file_server = Xerela::ConnectionPath::FileServer->new(-protocol => "TFTP", -ip => "10.10.10.10", -port => "69", -rootDir => "/home/someuser/tfp");
	my $ip_address = $tftp_file_server->get_ip_address();

=head1 DESCRIPTION

The C<Xerela::ConnectionPath::FileServer> module provides a simple way to store and retreive metadata/information about an
external file server.  A file server could be any TFTP, FTP, etc. server that may be located on the same machine running Xerela
...
Xerela.  This means that either the file server must be running on the same machine, or there must be a mapping to the directory
...
Creates an array of new instances of the C<Xerela::ConnectionPath::FileServer> module by parsing an input XML string that represents
...
Creates a new C<Xerela::ConnectionPath::FileServer> instance.  If no arguments are specified, then all of the internal
