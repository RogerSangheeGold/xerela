package Xerela::Adapters::Nortel::Tiara::AutoLogin;
...
sub _initial_connection
{
	...
	my @responses = ();
	push(@responses, Xerela::Response->new('sername:|login:|login as:', \&_send_username));
	push(@responses, Xerela::Response->new('[Pp]assword:', \&_send_password));
	push(@responses, Xerela::Response->new('(?m)^\s*\S+(#|\$|>)\s*$', \&_get_prompt));
...
sub _send_username
{
	...
	my @responses = ();
	push(@responses, Xerela::Response->new('[Pp]assword:', \&_send_password));
	push(@responses, Xerela::Response->new('login|denied', undef, $INVALID_CREDENTIALS));
...
sub _send_password
{
	...
	# Specify the responses to check for
	my @responses = ();
	push(@responses, Xerela::Response->new('incorrect|denied', undef, $INVALID_CREDENTIALS));
