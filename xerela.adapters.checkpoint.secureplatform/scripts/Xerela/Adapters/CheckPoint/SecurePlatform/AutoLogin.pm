package Xerela::Adapters::CheckPoint::SecurePlatform::AutoLogin;
...
sub _initial_connection
{
	...
	my @responses = ();
	push( @responses, Xerela::Response->new( 'sername:|(?<!Last)\s+login:', \&_send_username ) );
	push( @responses, Xerela::Response->new( '[Pp]assword:',                \&_send_password ) );
	push( @responses, Xerela::Response->new( '(#|\$|>)\s*$',                \&_expert ) );
...
sub _send_password
{
	...
	# Specify the responses to check for
	my @responses = ();
	push( @responses, Xerela::Response->new( 'incorrect|login invalid', undef, $INVALID_CREDENTIALS ) );
	push( @responses, Xerela::Response->new( '(#|\$|>)\s*$', \&_expert ) );
...
sub _expert
{
	...
	push( @responses, Xerela::Response->new( 'password:',    \&_send_expert_password ) );
...
sub _send_expert_password
{
	...
	# Specify the responses to check for
	my @responses = ();
	push( @responses, Xerela::Response->new( 'Wrong password', undef, $INVALID_CREDENTIALS ) );
	push( @responses, Xerela::Response->new( '(#|\$|>)\s*$', \&_get_prompt ) );
