...
import org.xerela.server.job.backup.IBackupPersisterEx;
import org.xerela.zap.util.Base64;
import org.xerela.zap.util.Base64.Base64OutputStream;
...
{
    public static final String XERELA_ELEMENT_DOCUMENT = "Xerela-Element-Document"; //$NON-NLS-1$
    ...
    static
    {
        ...
        pathsOfInterest.add("/XerelaElementDocument/configRepository"); //$NON-NLS-1$
        pathsOfInterest.add("/XerelaElementDocument/configRepository/*"); //$NON-NLS-1$
        pathsOfInterest.add("/XerelaElementDocument/lastReboot"); //$NON-NLS-1$
    ...
    private void createZed()
    {
        try
        {
            File outputFile = File.createTempFile(XERELA_ELEMENT_DOCUMENT, ".xml"); //$NON-NLS-1$
            ConfigHolder configHolder = new ConfigHolder(outputFile, '/' + XERELA_ELEMENT_DOCUMENT, "text/xml"); //$NON-NLS-1$
        ...
        {
            throw new RuntimeException("Error copying Xerela Element Document to configuration repository.", e); //$NON-NLS-1$
        }
        catch (XMLStreamException e)
        {
            throw new RuntimeException("Error copying Xerela Element Document to configuration repository.", e); //$NON-NLS-1$
