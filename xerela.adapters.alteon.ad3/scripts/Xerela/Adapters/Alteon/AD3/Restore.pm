package Xerela::Adapters::Alteon::AD3::Restore;
...
sub restore_via_tftp
{
	...
	my @responses = ();
	push(@responses, Xerela::Response->new('successfully tftp\'d', \&_finish));	
	push(@responses, Xerela::Response->new('Enter hostname or IP address of TFTP server:', \&_specify_tftp_address));
	push(@responses, Xerela::Response->new('Enter name of file on TFTP server:', \&_specify_source_file));
	push(@responses, Xerela::Response->new('Timeout|File not found', undef, $TFTP_ERROR));
...
sub _specify_tftp_address
{
	...
	push(@responses, Xerela::Response->new('Enter name of file on TFTP server:', \&_specify_source_file));
	push(@responses, Xerela::Response->new('successfully tftp\'d', \&_finish));	
	push(@responses, Xerela::Response->new('Timeout|File not found|Enter hostname or IP address of TFTP server:', undef, $TFTP_ERROR));
...
sub _specify_source_file
{
	...
	my @responses = ();
	push(@responses, Xerela::Response->new('successfully tftp\'d', \&_finish));	
	push(@responses, Xerela::Response->new('Timeout|File not found|Enter hostname or IP address of TFTP server:|Enter name of file on TFTP server:', undef, $TFTP_ERROR));
