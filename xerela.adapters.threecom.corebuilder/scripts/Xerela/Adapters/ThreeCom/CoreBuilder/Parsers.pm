package Xerela::Adapters::ThreeCom::CoreBuilder::Parsers;

use strict;
use warnings;
use Xerela::Addressing::Subnet;
use Xerela::Adapters::Utils qw(mask_to_bits seconds_since_epoch get_mask get_port_number trim get_interface_type get_crep strip_mac getUnitFreeNumber);
...
sub parse_interfaces
{
	...
	while ($blob1 =~ /(\d+)([^\n]*)/mig)
	{
		...
		if ( $ports_ips->{$id1} )
		{
			...
			my $subnet = new Xerela::Addressing::Subnet( $ports_ips->{$id1}->{ipAddress}, $ports_ips->{$id1}->{mask} );
