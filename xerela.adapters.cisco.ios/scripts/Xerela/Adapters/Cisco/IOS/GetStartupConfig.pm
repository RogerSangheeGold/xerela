package Xerela::Adapters::Cisco::IOS::GetStartupConfig;
...
sub _get_startup_config_tftp
{
	...
	push(@responses, Xerela::Response->new('[Ss]ource [Ff]ilename', \&_specify_source_startup_file));
	...
	push(@responses, Xerela::Response->new($bad_command_regex));
...
sub _specify_tftp_address
{
	...
	my @responses = ();
	push(@responses, Xerela::Response->new('%Error', undef, $TFTP_ERROR));
	push(@responses, Xerela::Response->new('filename|file to write', \&_specify_startup_config_name));
...
sub _specify_startup_config_name
{
	...
	push(@responses, Xerela::Response->new('bytes copied|OK|#', \&_finish));
