package Xerela::Adapters::Cisco::IOS;
...
use Xerela::Adapters::Cisco::IOS::GetRunningConfig qw(get_running_config);
use Xerela::Adapters::Cisco::IOS::GetStartupConfig qw(get_startup_config);
...
use Xerela::Adapters::Cisco::IOS::RestoreStartupConfig;
...
sub restore
{
	...
	if ( $restoreFile->get_path() =~ /startup-config/i )
	{
		...
		{
			my ( $cli_protocol, $enable_prompt_regex ) = _connect($connection_path);
			Xerela::Adapters::Cisco::IOS::RestoreStartupConfig::restore_via_tftp( $connection_path, $cli_protocol, $restoreFile );
			disconnect($cli_protocol);
		}
		elsif ( defined($scp_protocol) )
		{
			Xerela::Adapters::GenericAdapter::scp_restore( $connection_path, $restoreFile );
		}
		else
		{
...
sub _connect
{
	...
	my $enable_prompt_regex = Xerela::Adapters::Cisco::IOS::AutoLogin::execute( $cli_protocol, $connection_path );
...
Xerela::Adapters::Cisco::IOS - Adapter for performing various operations against Cisco IOS devices.

=head1 SYNOPSIS

    use Xerela::Adapters::Cisco::IOS;
	Xerela::Adapters::Cisco::IOS::backup( $backup_document );
