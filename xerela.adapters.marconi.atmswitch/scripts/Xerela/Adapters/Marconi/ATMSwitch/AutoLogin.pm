package Xerela::Adapters::Marconi::ATMSwitch::AutoLogin;
...
sub _initial_connection
{
	...
	my @responses = ();
	push(@responses, Xerela::Response->new('sername:|login:', \&_send_username));
...
sub _send_password
{
	...
	# Specify the responses to check for
	my @responses = ();
	push(@responses, Xerela::Response->new('sername:|login:|Maximum login retries exceeded', undef, $INVALID_CREDENTIALS));
	push(@responses, Xerela::Response->new('->\s*$', \&_get_prompt));
