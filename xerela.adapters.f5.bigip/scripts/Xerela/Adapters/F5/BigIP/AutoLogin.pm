package Xerela::Adapters::F5::BigIP::AutoLogin;
...
sub _initial_connection
{
	...
	#push(@responses, Xerela::Response->new('continue connecting \(yes\/no\)\?\s*$', \&_send_confirmation));
...
Xerela::Adapters::F5::BigIP::AutoLogin - Automates the connection and authentication with a F5 BigIP device.

=head1 SYNOPSIS

    use Xerela::Adapters::F5::BigIP;
	my $cli_protocol = Xerela::CLIProtocolFactory::create( $connection_path );
	my $prompt_regex = Xerela::Adapters::F5::BigIP::execute( $cli_protocol, $connection_path );
