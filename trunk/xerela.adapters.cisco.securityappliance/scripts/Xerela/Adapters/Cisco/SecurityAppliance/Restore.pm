package Xerela::Adapters::Cisco::SecurityAppliance::Restore;
...
sub _asa_restore
{
	...
	my @responses = ();
	push( @responses, Xerela::Response->new( '[Ee]rror|[Ff]ail|Invalid input', undef, $TFTP_ERROR ) );
	push( @responses, Xerela::Response->new( 'Address or name of remote host', \&_send_tftp_address ) );
	push( @responses, Xerela::Response->new( 'Source filename',                \&_send_source_filename ) );
	push( @responses, Xerela::Response->new('\d+ bytes copied in \S+ secs') );
...
sub _send_tftp_address
{
	...
	my @responses = ();
	push( @responses, Xerela::Response->new( '[Ee]rror|[Ff]ail|Address or name of remote host', undef, $TFTP_ERROR ) );
	push( @responses, Xerela::Response->new( 'Source filename', \&_send_source_filename ) );
	push( @responses, Xerela::Response->new('\d+ bytes copied in \S+ secs') );
...
sub _send_source_filename
{
	...
	my @responses = ();
	push( @responses, Xerela::Response->new( '[Ee]rror|[Ff]ail|Address or name of remote host|Source filename', undef, $TFTP_ERROR ) );
	push( @responses, Xerela::Response->new( '\d+ bytes copied in \S+ secs', \&_finish ) );
