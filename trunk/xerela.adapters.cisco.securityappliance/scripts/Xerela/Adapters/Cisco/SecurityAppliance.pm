package Xerela::Adapters::Cisco::SecurityAppliance;
...
use Xerela::Adapters::Cisco::SecurityAppliance::GetConfig qw(get_config);
use Xerela::Adapters::Cisco::SecurityAppliance::Parsers
...
use Xerela::Adapters::Cisco::SecurityAppliance::Disconnect
	qw(disconnect);
use Xerela::Adapters::Cisco::SecurityAppliance::Restore;
...
sub backup
{
	...
	{
		my $systemPrompt = Xerela::Adapters::Cisco::SecurityAppliance::AutoLogin::_get_prompt( $cli_protocol );
...
sub commands
{
	...
	my $result = Xerela::Adapters::GenericAdapter::execute_cli_commands('SecurityAppliance', $cli_protocol, $commands, $enable_prompt_regex.'|(#|\$|>)\s*$');
	disconnect($cli_protocol);
	return $result;
}
...
sub restore
{
	...
	Xerela::Adapters::Cisco::SecurityAppliance::Restore::execute( $connection_path, $cli_protocol, $enable_prompt_regex, $restoreFile );
...
sub _connect
{
	...
	my $enable_prompt_regex = Xerela::Adapters::Cisco::SecurityAppliance::AutoLogin::execute( $cli_protocol, $connection_path );
	...
	# Return the created Xerela::CLIProtocol object and the enable prompt encountered after successfully connecting to a device.
...
Xerela::Adapters::Cisco::SecurityAppliance - Adapter for performing various operations against Cisco Security Appliance devices.

=head1 SYNOPSIS

    use Xerela::Adapters::Cisco::SecurityAppliance;
	Xerela::Adapters::Cisco::SecurityAppliance::backup( $backup_document );
