package Xerela::Adapters::Nortel::Tiara::Restore;
...
sub _connect
{
	...
	my $device_prompt_regex = Xerela::Adapters::Nortel::Tiara::AutoLogin::execute( $cli_protocol, $connection_path );
...
sub restore_via_tftp
{
	...
	my @responses = ();
	push( @responses, Xerela::Response->new( 'host\s+:',					\&_send_tftp_ip ) );
	push( @responses, Xerela::Response->new( 'remote file\s+:|fileName\s+:',\&_send_source_filename ) );
	push( @responses, Xerela::Response->new( 'could not configure',		undef, $TFTP_ERROR ) );
...
sub _send_tftp_ip
{
	...
	my @responses = ();
	push( @responses, Xerela::Response->new( 'remote file\s+:|fileName\s+:',		\&_send_source_filename ) );
	push( @responses, Xerela::Response->new( 'could not configure|host\s+:',		undef, $TFTP_ERROR ) );
...
sub _send_source_filename
{
	...
	my @responses = ();
	push( @responses, Xerela::Response->new( 'Total commands executed', \&_finish ) );
	push( @responses, Xerela::Response->new( 'could not configure|host\s+:|remote file\s+:|fileName\s+:', undef, $TFTP_ERROR ) );
