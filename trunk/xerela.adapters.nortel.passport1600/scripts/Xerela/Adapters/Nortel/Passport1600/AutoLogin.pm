package Xerela::Adapters::Nortel::Passport1600::AutoLogin;
...
sub _initial_connection
{
	...
	push( @responses, Xerela::Response->new( '3#\s*$', \&_send_enable ) );
	push( @responses, Xerela::Response->new( '4#\s*$', \&_get_prompt ) );
...
sub _send_enable
{
	...
	{    
		# Specify the responses to check for
		my @responses = ();
		push( @responses, Xerela::Response->new( 'Fail!', undef, $INVALID_CREDENTIALS ) );
...
sub _send_enable_password
{
	...
	# Specify the responses to check for
	my @responses = ();
	push( @responses, Xerela::Response->new( '[Pp]assword:', undef, $INVALID_CREDENTIALS ) );
	push( @responses, Xerela::Response->new( '4#\s*$', \&_get_prompt ) );
