package Xerela::Adapters::Nortel::Accelar::Restore;
...
sub restore_via_tftp
{
	...
	my @responses = ();
	push( @responses, Xerela::Response->new( '[Ee]nter source tftp server address',			\&_send_tftp_ip ) );
	push( @responses, Xerela::Response->new( '[Ee]nter source file',						\&_send_source_filename ) );
	push( @responses, Xerela::Response->new( '[Ee]rror',	undef, $TFTP_ERROR ) );
...
sub _send_tftp_ip
{
	...
	my @responses = ();
	push( @responses, Xerela::Response->new( '[Ee]nter source file',							\&_send_source_filename ) );
	push( @responses, Xerela::Response->new( '[Ee]rror|[Ee]nter source tftp server address',	undef, $TFTP_ERROR ) );
...
sub _send_source_filename
{
	...
	my @responses = ();
	push( @responses, Xerela::Response->new( 'tftp result: success', \&_finish ) );
	push( @responses, Xerela::Response->new( '[Ee]rror|[Ee]nter source tftp server address|[Ee]nter source file', undef, $TFTP_ERROR ) );
