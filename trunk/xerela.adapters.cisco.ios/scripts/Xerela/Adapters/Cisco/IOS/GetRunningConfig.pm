package Xerela::Adapters::Cisco::IOS::GetRunningConfig;
...
sub _get_running_config_tftp
{
	...
	push(@responses, Xerela::Response->new('[Ss]ource [Ff]ilename', \&_specify_source_running_file));
	...
	push(@responses, Xerela::Response->new($bad_command_regex, \&_copy_nvram));
...
sub _copy_nvram()
{
	...
	my @responses = ();
	push(@responses, Xerela::Response->new('memory invalid', undef, $DEVICE_MEMORY_ERROR));
	push(@responses, Xerela::Response->new('Error: Configuration upload operation failed', undef, $TFTP_ERROR));
	push(@responses, Xerela::Response->new('Invalid input', undef, $INSUFFICIENT_PRIVILEGE));
...
sub _specify_tftp_address
{
	...
	my @responses = ();
	push(@responses, Xerela::Response->new('%Error', undef, $TFTP_ERROR));
	push(@responses, Xerela::Response->new('filename|file to write', \&_specify_running_config_name));
...
sub _specify_running_config_name
{
	...
	push(@responses, Xerela::Response->new('memory invalid', undef, $DEVICE_MEMORY_ERROR));
	push(@responses, Xerela::Response->new('Error opening nvram', undef, $NVRAM_CORRUPTION_ERROR));
	push(@responses, Xerela::Response->new('confirm', \&_confirm_tftp));
	push(@responses, Xerela::Response->new($enable_prompt, \&_finish));
