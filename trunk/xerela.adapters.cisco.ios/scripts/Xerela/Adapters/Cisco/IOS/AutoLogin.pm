...
sub _initial_connection
{
	...
	push( @responses, Xerela::Response->new( 'assword required, but none se', undef, $PASSWORD_REQUIRED_BUT_NOT_SET ) );
	push( @responses, Xerela::Response->new( 'sername:',            \&_send_username ) );
	push( @responses, Xerela::Response->new( 'assword:',            \&_send_password ) );
	push( @responses, Xerela::Response->new( 'PASSCODE:',           \&_send_password ) );
	push( @responses, Xerela::Response->new( '(?m)^\w\S*>\s*(?![\n\r])$',       \&_send_enable ) );
	push( @responses, Xerela::Response->new( '(?m)^\w\S*#\s*(?![\n\r])$',       \&_send_enable ) );
	push( @responses, Xerela::Response->new( 'any key',             \&_send_press_any_key ) );       
	push( @responses, Xerela::Response->new( 'User Interface Menu', \&_select_command_line ) );
...
sub _send_username
{
	...
	# Specify the responses to check for
	my @responses = ();
	push( @responses, Xerela::Response->new( 'invalid', undef, $INVALID_CREDENTIALS ) );
	push( @responses, Xerela::Response->new( 'assword:|PASSCODE:', \&_send_password ) );
...
sub _send_password
{
	...
	# Specify the responses to check for
	my @responses = ();
	push( @responses, Xerela::Response->new( 'sername:',              undef, $INVALID_CREDENTIALS ) );
	push( @responses, Xerela::Response->new( 'assword:',              undef, $INVALID_CREDENTIALS ) );
	push( @responses, Xerela::Response->new( 'PASSCODE:',             undef, $INVALID_CREDENTIALS ) );
	push( @responses, Xerela::Response->new( 'invalid',               undef, $INVALID_CREDENTIALS ) );
	push( @responses, Xerela::Response->new( 'Bad passwords',         undef, $INVALID_CREDENTIALS ) );
	push( @responses, Xerela::Response->new( 'Authentication failed', undef, $INVALID_CREDENTIALS ) );
	push( @responses, Xerela::Response->new( '(\d+)\s+[Ee][Xx][Ii][Tt]\s+.*[Mm][Ee][Nn][Uu]',     \&_handle_term_svr_menu ) );
	push( @responses, Xerela::Response->new( '>\s*$',                                             \&_send_enable ) );
	push( @responses, Xerela::Response->new( '(^|\n|\r)[^#^\n^\r]+#\s*$|[^#^\n^\r]+#\s*\S+#\s*$', \&_send_enable ) );
	push( @responses, Xerela::Response->new( '.*#.*#',                                            \&_send_enable ) );
...
sub _send_enable
{
	...
	{
		my @responses = ();
		push( @responses, Xerela::Response->new( 'sername:',                                          \&_re_enter_username_during_enable ) );
		push( @responses, Xerela::Response->new( 'assword:',                                          \&_send_enable_password ) );
		push( @responses, Xerela::Response->new( 'PASSCODE:',                                         \&_send_enable_password ) );
...
sub _send_enable_password
{
	...
	push( @responses, Xerela::Response->new( 'Bad passwords',                  undef, $INVALID_CREDENTIALS ) );
	push( @responses, Xerela::Response->new( 'Authentication failed',          undef, $INVALID_CREDENTIALS ) );
	push( @responses, Xerela::Response->new( '%\s+[Ee]rror in authentication', undef, $INVALID_CREDENTIALS ) );
	push( @responses, Xerela::Response->new( '(^|\n|\r)[^#^\n^\r]+#\s*$|[^#^\n^\r]+#\s*\S+#\s*$', \&_calculate_prompt_regex ) );
...
sub _select_command_line
{
	...
	my @responses = ();
	push( @responses, Xerela::Response->new( 'sername',                                           \&_send_username ) );
	push( @responses, Xerela::Response->new( 'CLI session',                                       \&_send_enable ) );
	push( @responses, Xerela::Response->new( '(^|\n|\r)[^>^(\n|\r)]+>\s*$',                       \&_send_enable ) );
	push( @responses, Xerela::Response->new( '(^|\n|\r)[^#^\n^\r]+#\s*$|[^#^\n^\r]+#\s*\S+#\s*$', \&_send_enable ) );
	push( @responses, Xerela::Response->new( 'assword',                                           \&_send_enable_password ) );
...
Xerela::Adapters::Cisco::IOS::AutoLogin - Automates the connection and authentication with a Cisco IOS device.

=head1 SYNOPSIS

    use Xerela::Adapters::Cisco::IOS::AutoLogin;
	my $cli_protocol = Xerela::CLIProtocolFactory::create( $connection_path );
	my $prompt_regex = Xerela::Adapters::Cisco::IOS::AutoLogin::execute( $cli_protocol, $connection_path );
