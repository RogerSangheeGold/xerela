package Xerela::Adapters::Nortel::BayStack::AutoLogin;
...
sub _initial_connection
{
    ...
    my @responses = ();
    push( @responses, Xerela::Response->new( 'Ctrl-Y',                \&_send_ctrl_y ) );
    push( @responses, Xerela::Response->new( 'ommand Line Interface', \&_enter_cli ) );
    push( @responses, Xerela::Response->new( 'ogout',                 \&_finish ) );
    push( @responses, Xerela::Response->new( 'ogin|sername',          \&_enter_user ) );
    push( @responses, Xerela::Response->new( 'assword',               \&_enter_pass ) );
...
sub _enter_user
{
	...
	if ($hitDownArrow == 1)
	{
		push( @responses, Xerela::Response->new( 'sername', \&_down_arrow ) );
	}
	elsif ($hitDownArrow == 0)
	{
		push( @responses, Xerela::Response->new( 'sername', \&_enter_pass ) );
	}
	push( @responses, Xerela::Response->new( 'assword', \&_enter_pass ) );
...
sub _down_arrow
{
	...
	# Specify the responses to check for
	my @responses = ();
	push( @responses, Xerela::Response->new( 'Enter', \&_enter_pass ) );
...
sub _enter_pass
{
	...
	# Specify the responses to check for
	my @responses = ();
	push( @responses, Xerela::Response->new( 'Access Denied from RADIUS|Incorrect', undef, $INVALID_CREDENTIALS ) );
	push( @responses, Xerela::Response->new( 'ommand Line Interface', \&_enter_cli ) );
	push( @responses, Xerela::Response->new( $PROMPT,              \&_capture_prompt ) );
	push( @responses, Xerela::Response->new( 'ogout',                 \&_finish ) );
...
sub _enter_cli
{
	...
	my @responses = ();
	push( @responses, Xerela::Response->new( $PROMPT, \&_capture_prompt ) );
	push( @responses, Xerela::Response->new( '(?m)^\w.+?>\s*$', \&_enable ) );
