package Xerela::Adapters::Cisco::ArrowPoint;
...
use Xerela::Adapters::Cisco::ArrowPoint::GetConfig qw(get_startup_config get_running_config get_boot_config);
use Xerela::Adapters::Cisco::ArrowPoint::RestoreStartupConfig;
use Xerela::Adapters::Cisco::ArrowPoint::AutoLogin;
use Xerela::Adapters::Cisco::ArrowPoint::Disconnect qw(disconnect);
use Xerela::Adapters::Cisco::ArrowPoint::Parsers
...
sub commands
{
	...
	my $termLen = $cli_protocol->send_and_wait_for( "no terminal more", $prompt_regex );
	my $result = Xerela::Adapters::GenericAdapter::execute_cli_commands( 'ArrowPoint', $cli_protocol, $commands, $prompt_regex.'|(#|\$|>)\s*$' );
	disconnect($cli_protocol);
	return $result;
}
...
sub restore
{
	...
	if ( $restoreFile->get_path() =~ /startup-config/i )
	{
		...
		{
			my ( $cli_protocol, $prompt_regex ) = _connect($connection_path);
			Xerela::Adapters::Cisco::ArrowPoint::RestoreStartupConfig::restore_via_tftp( $connection_path, $cli_protocol, $restoreFile, $prompt_regex );
			disconnect($cli_protocol);
		}
		else
		{
...
sub _connect
{
	...
	my $device_prompt_regex = Xerela::Adapters::Cisco::ArrowPoint::AutoLogin::execute( $cli_protocol, $connection_path );
...
Xerela::Adapters::Cisco::ArrowPoint - Example adapter for performing various operations against a particular family of devices.

=head1 SYNOPSIS

    use Xerela::Adapters::Cisco::ArrowPoint;
	Xerela::Adapters::Cisco::ArrowPoint::backup( $backup_document );
