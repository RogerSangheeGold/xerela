package Xerela::Adapters::Cisco::ArrowPoint::GetConfig;
...
sub _get_file_tftp
{
	...
	my @responses = ();
	push(@responses, Xerela::Response->new($regex, sub { _finish($cli_protocol,$connection_path,$file) }));
	push(@responses, Xerela::Response->new('(?m)^%%\s+Could',undef, $TFTP_ERROR));
