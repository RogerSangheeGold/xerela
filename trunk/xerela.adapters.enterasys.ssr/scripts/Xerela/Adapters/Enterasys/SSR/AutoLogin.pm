package Xerela::Adapters::Enterasys::SSR::AutoLogin;
...
sub _initial_connection
{
	...
	my @responses = ();
	push(@responses, Xerela::Response->new('sername:|[Ll]ogin:', \&_send_username));
...
sub _send_username
{
	...
	my @responses = ();
	push(@responses, Xerela::Response->new('[Pp]assword:', \&_send_password));
	push(@responses, Xerela::Response->new('incorrect|Authentication failed', undef, $INVALID_CREDENTIALS));
...
sub _send_password
{
	...
	# Specify the responses to check for
	my @responses = ();
	push(@responses, Xerela::Response->new('sername:|Authentication failed', undef, $INVALID_CREDENTIALS));
	push(@responses, Xerela::Response->new('>\s*$', \&_send_enable));
...
sub _send_enable
{
	...
	{    
		# Specify the responses to check for
		my @responses = ();
		push( @responses, Xerela::Response->new( 'incorrect password|Authentication failed', undef, $INVALID_CREDENTIALS ) );
		push( @responses, Xerela::Response->new( 'sername:', \&_send_enable_username ) );
...
sub _send_enable_username
{
	...
	# Specify the responses to check for
	my @responses = ();
	push( @responses, Xerela::Response->new( 'sername:|incorrect password|Authentication failed', undef, $INVALID_CREDENTIALS ) );
	push( @responses, Xerela::Response->new( '[Pp]assword:', \&_send_enable_password ) );
	push( @responses, Xerela::Response->new( '#\s*$', \&_get_prompt ) );
