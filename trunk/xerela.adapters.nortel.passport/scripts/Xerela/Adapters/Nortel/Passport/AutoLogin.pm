package Xerela::Adapters::Nortel::Passport::AutoLogin;
...
sub _send_password
{
	...
	# Specify the responses to check for
	my @responses = ();
	push(@responses, Xerela::Response->new('incorrect|invalid|Login:', undef, $INVALID_CREDENTIALS));
...
sub _send_enable
{
	...
	# Specify the responses to check for
	my @responses = ();
    push(@responses, Xerela::Response->new('[pP]assword:', \&_send_enable_pass));
    push(@responses, Xerela::Response->new('(#|\$)\s*$', \&_get_prompt));
...
sub _send_enable_pass
{
	...
	# Specify the responses to check for
	my @responses = ();
	push(@responses, Xerela::Response->new('assword|incorrect|invalid', undef, $INVALID_CREDENTIALS));
    push(@responses, Xerela::Response->new('(#|\$)\s*$', \&_get_prompt));
...
Xerela::Adapters::Nortel::Passport::AutoLogin - An example autologin module

=head1 SYNOPSIS

    use Xerela::Adapters::Nortel::Passport;
	my $cli_protocol = Xerela::CLIProtocolFactory::create( $connection_path );
	my $prompt_regex = Xerela::Adapters::Nortel::Passport::execute( $cli_protocol, $connection_path );
