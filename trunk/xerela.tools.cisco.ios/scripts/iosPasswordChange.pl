...
use Xerela::Typer;
use Xerela::Logger;
use Xerela::Tools::ServerElf qw(update_credential update_dependent_credential);
...
# Store the connection path XML and the Xerela::ConnectionPath object
...
while ($_ = shift(@ARGV))
{
   ...
   {
       $connectionPathXML = $';
       ($connectionPath) = Xerela::Typer::translate_document( $connectionPathXML, 'connectionPath' );
   }
}
...
eval
{
	...
	my $enablePrompt = Xerela::Adapters::Cisco::IOS::AutoLogin::execute( $cliProtocol, $connectionPath );
