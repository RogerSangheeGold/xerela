package Xerela::Adapters::Nortel::BayRS::AutoLogin;
...
sub _initial_connection
{
	...
	push(@responses, Xerela::Response->new('(#|\$)\s*$', \&_send_bcc));
	push(@responses, Xerela::Response->new('bcc\s*>\s*$', \&_get_prompt));
...
sub _send_username
{
	...
	my @responses = ();
	push(@responses, Xerela::Response->new('[Pp]assword:', \&_send_password));
	#push(@responses, Xerela::Response->new('Nortel\s+Networks\,\s+Inc\.', undef, $INVALID_CREDENTIALS));
	#push(@responses, Xerela::Response->new('Bay\s+Networks\,\s+Inc\.', undef, $INVALID_CREDENTIALS));
...
sub _send_password
{
	...
	# Specify the responses to check for
	my @responses = ();
	push(@responses, Xerela::Response->new('sername:|ogin:', undef, $INVALID_CREDENTIALS));
	push(@responses, Xerela::Response->new('Nortel\s+Networks\,\s+Inc\.', undef, $INVALID_CREDENTIALS));
	push(@responses, Xerela::Response->new('Bay\s+Networks\,\s+Inc\.', undef, $INVALID_CREDENTIALS));
	push(@responses, Xerela::Response->new('(#|\$)\s*$', \&_send_bcc));
	push(@responses, Xerela::Response->new('bcc\s*>\s*$', \&_get_prompt));
...
sub _send_bcc
{
	...
	# Specify the responses to check for
	my @responses = ();
	push(@responses, Xerela::Response->new('Unable to load bcc command from file|BCC\s+requires\s+\d+\s+Mb\s+of\s+free\s+memory|(?:#|\$)\s*$', undef, $BCC_FAILED));
	push(@responses, Xerela::Response->new('bcc\s*>\s*$', \&_get_prompt));
...
Xerela::Adapters::Nortel::BayRS::AutoLogin - An example autologin module

=head1 SYNOPSIS

    use Xerela::Adapters::Nortel::BayRS;
	my $cli_protocol = Xerela::CLIProtocolFactory::create( $connection_path );
	my $prompt_regex = Xerela::Adapters::Nortel::BayRS::execute( $cli_protocol, $connection_path );
