package Xerela::Adapters::Cisco::CatOS::RestoreConfig;
...
sub restore_via_tftp
{
	...
	my @responses = ();
	push( @responses, Xerela::Response->new( 'address or name of remote', \&_send_tftp_ip ) );
	push( @responses, Xerela::Response->new( 'File system in use',        undef, $DEVICE_MEMORY_ERROR ) );
...
sub _send_tftp_ip
{
	...
	my @responses = ();
	push( @responses, Xerela::Response->new( 'Flash device', \&_send_bootflash ) );
	push( @responses, Xerela::Response->new( 'is unrecognized|TFTP connection fail', undef, $TFTP_ERROR ) );
...
sub _send_bootflash
{
	...
	$cliProtocol->send_and_wait_for( 'xerelaconfig.cfg', 'proceed|Overwrite auto-config' );
...
sub _send_yes
{
	...
	my @responses = ();
	push( @responses, Xerela::Response->new( 'proceed \(y\/n\)',                                                            \&_send_yes ) );
	push( @responses, Xerela::Response->new( $promptRegex,                                                                  \&_squeeze ) );
	push( @responses, Xerela::Response->new( 'too many recursions|No response from host|File not found|File system in use', undef, $TFTP_ERROR ) );
...
sub _make_config_live
{
	my ( $cliProtocol, $promptRegex ) = @_;
	my @responses = ();
	push( @responses, Xerela::Response->new( 'CONFIG_FILE|Unknown', \&_copy_config ) );
	push( @responses, Xerela::Response->new( 'invalid config|not a valid',   undef, $DEVICE_MEMORY_ERROR ) );    

	$cliProtocol->send('set boot auto-config bootflash:xerelaconfig.cfg');
	my $response = $cliProtocol->wait_for_responses( \@responses );
	if ($response)
	{
		my $nextMethod = $response->get_next_interaction();
...
{
	my ( $cliProtocol, $promptRegex ) = @_;
	my $response = $cliProtocol->send_and_wait_for('copy xerelaconfig.cfg config', "Configure using bootfla|$promptRegex", 120);
