...
use Xerela::Adapters::Cisco::CatOS::Disconnect
	qw(disconnect);
...
sub commands
{
	...
	my $result = Xerela::Adapters::GenericAdapter::execute_cli_commands('CatOS', $cli_protocol, $commands, '(#|\$|>)\s*$|'.$enable_prompt_regex);
	disconnect($cli_protocol);
	return $result;
}
...
sub restore
{
	...
	if ( $restoreFile->get_path() =~ /config/i )
	{
		...
		{
			my ( $cli_protocol, $enable_prompt_regex ) = _connect($connection_path);
			Xerela::Adapters::Cisco::CatOS::RestoreConfig::restore_via_tftp( $connection_path, $cli_protocol, $restoreFile, $enable_prompt_regex );
			disconnect($cli_protocol);
		}
...
sub _connect
{
	...
	my $enable_prompt_regex = Xerela::Adapters::Cisco::CatOS::AutoLogin::execute( $cli_protocol, $connection_path );
