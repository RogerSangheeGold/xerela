...
Xerela::SNMP - Allows easy execution of basic SNMP actions using Net::SNMP as the back-end.

=head1 SYNOPSIS

	use Xerela::SNMP;
	my $resultsHash = Xerela::SNMP::get($snmpSession, $var_bind_list_ref);
	my $resultsHash = Xerela::SNMP::set($snmpSession, $var_bind_list_ref);
	my $resultsHash = Xerela::SNMP::walk($snmpSession, $root_oid);

=head1 DESCRIPTION

C<Xerela::SNMP> provides an easy way to execute the following SNMP actions: C<GET>, C<SET>, and C<WALK> (similar to get_next and get_bulk).

Leveraging the power of the C<Net::SNMP> module, only a valid C<Net::SNMP> session, in addition to the necessary parameters,
are needed to execute each action.  In order to create a valid C<Net::SNMP> session, you can use the C<Xerela::SnmpSessionFactory>
module in conjunction with valid C<Xerela::ConnectionPath> and C<Xerela::Credentials> objects to create a valid C<Net::SNMP> session
