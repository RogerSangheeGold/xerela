package Xerela::Adapters::GenericAdapter;
...
use Switch;
use Xerela::Adapters::Utils qw(mask_to_bits get_cli_commands_filehandle close_model_filehandle get_crep);
use Xerela::Logger;
...
sub execute_cli_commands
{
	...
	my $printer     = Xerela::Model::XmlPrint->new($filehandle);
	$printer->open_element('XerelaDeviceCommmandDetails') if $printStdout;
	...
	$printer->close_element('XerelaDeviceCommmandDetails') if $printStdout;
	return $results;
}
...
sub get_interfaces
{
	...
	use Xerela::SNMP;
	my $r1 = Xerela::SNMP::walk( $session, $ifDescr );
	...
	my $r2 = Xerela::SNMP::walk( $session, $ifType );
	...
	my $r3 = Xerela::SNMP::walk( $session, $ifMtu );
	...
	my $r4 = Xerela::SNMP::walk( $session, $ifSpeed );
	...
	my $r5 = Xerela::SNMP::walk( $session, $ifPhysAddress );
	...
	my $r6 = Xerela::SNMP::walk( $session, $ifAdminStatus );
	...
	my $results = Xerela::SNMP::walk( $session, $ipAddrTable );
...
# given an ifType value, match it to one of the allowed Xerela interface types
sub resolve_type
{
	my $ifType   = shift;
	my $type     = "other";
...
Xerela::Adapters::GenericAdapter

=head1 SYNOPSIS

    use Xerela::Adapters::GenericAdapter;
	Xerela::Adapters::GenericAdapter::execute_cli_commands($cli_protocol, $commands, $defaultPrompt);

=head1 DESCRIPTION

This module contains methods that can be of use to any adapter.  For example, they use
the Xerela::CliProtocol or use public SNMP mibs through the Xerela::SNMP module.
...
Runs a series of CLI based commands against the provided $cli_protocol object.
	...
	$cli_protocol - Implementation of Xerela::CLIProtocol
	$commands - Implementation of Xerela::CommandSet
...
as explained by the SNMP element of the XerelaElementDocument XSD.
...
Does an SCP 'put' of the provided $restoreFile
	$connectionPath - an instance of Xerela::ConnectionPath
	$restoreFile - an instance of Xerela::RestoreFile
