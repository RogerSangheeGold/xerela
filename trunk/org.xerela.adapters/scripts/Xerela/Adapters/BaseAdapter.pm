package Xerela::Adapters::BaseAdapter;
...
use Xerela::Adapters::Utils qw(choose_admin_ip mask_to_bits get_model_filehandle close_model_filehandle);
use Xerela::Logger;
...
sub telemetry
{
	...
	my $systemInfo = Xerela::Adapters::GenericAdapter::get_snmp($snmpSession);
...
sub _cisco_mac_table
{
	...
	my @macEntries        = ();
	my $foundVlans        = 0;
	my $vlans             = Xerela::SNMP::walk( $session, $CISCO_VLAN_NAMES );
	while ( my ( $key, $value ) = each %$vlans )
	{
		...
		if ( $key =~ /(\d+)$/ )
		{
			...
			my $tempSession = Xerela::SnmpSessionFactory->create($connectionPath);
...
sub _standard_mac_table
{
	...
	my $macTable = Xerela::SNMP::walk( $session, $MAC_TABLE );
...
sub _get_bridge_port_to_if_index
{
	...
	# for a bridge port
	my $session     = shift;
	my $bridgePorts = Xerela::SNMP::walk( $session, $BRIDGE_PORT_IFINDEX );
	my $map         = {};
...
sub _arp_table
{
	...
	my $arpTable = Xerela::SNMP::walk( $session, $ARP_ENTRIES );
	my $opened = 0;
...
sub _cdp_neighbors
{
	...
	if ( $sysOid =~ /$CISCO_EID_REGEX/ )
	{
		my $cdpCache     = Xerela::SNMP::walk( $session, $CDP_CACHE );
		my $cdpNeighbors = {};
		my $openElement  = 0;
...
sub _routing_neighbors
{
	...
	my $forwarding = Xerela::SNMP::get( $session, [$FORWARDING] );
...
sub _bgp_neighbors
{
	...
	my $bgpPeers = Xerela::SNMP::walk( $session, $BGP_PEER_IDENTIFIER );
...
{
	my ( $printer, $session, $interfaces ) = @_;
	my $ospfRouterIds   = Xerela::SNMP::walk( $session, $OSPF_NEIGHBORS_ROUTER_ID );
	my $ospfNeighbors   = Xerela::SNMP::walk( $session, $OSPF_NEIGHBORS );
	my $ospfAddressless = Xerela::SNMP::walk( $session, $OSPF_NEIGHBORS_IFINDEX );
		...
		foreach my $key ( sort ( keys(%$interfaces) ) )
		{
			if ( defined $interfaces->{$key}->{ipEntry} )
			{
				...
				foreach my $ip (@ips)
				{
					if ( defined $ip->{mask} )
					{
						my $subnet = new Xerela::Addressing::Subnet($ip->{ipAddress}, $ip->{mask});
...
{
	my ( $printer, $session, $interfaces ) = @_;
	my $peerAddr    = Xerela::SNMP::walk( $session, $EIGRP_PEER_ADDR );
	my $peerIfIndex = Xerela::SNMP::walk( $session, $EIGRP_PEER_IF_INDEX );
...
sub _populate_ip_entries
{
	...
	my $ifIndexes = Xerela::SNMP::walk( $session, $IP_AD_ENT_IF_INDEX );
	my $netmasks  = Xerela::SNMP::walk( $session, $IP_AD_ENT_NETMASK );
...
sub _populate_int_field
{
	...
	my $result = Xerela::SNMP::walk( $session, $oid );
	foreach my $key ( sort ( keys(%$result) ) )
	{
		if ( $key =~ /(\d+)$/ )
		{
			my $value = $result->{$key};
			($value) = Xerela::Adapters::GenericAdapter::resolve_type($value) if ( $fieldName eq 'type' );
