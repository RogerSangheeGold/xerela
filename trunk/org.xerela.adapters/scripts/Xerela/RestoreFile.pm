...
sub from_xml
{
	my $in_value = shift or $LOGGER->fatal("ERROR - No XML specified to convert into a Xerela::RestoreFile object!");
	my ($fullPath) = $in_value =~ /fullPathOnDevice=['"](.+?)['"]>/; 
	my ($blob) = $in_value =~ /base64EncodedFileBlob>(.*)<\/base64EncodedFileBlob/s; 
	return new Xerela::RestoreFile->new($fullPath, decode_base64($blob));
}
...
Xerela::RestoreFile - A configuration file to restore to a device
