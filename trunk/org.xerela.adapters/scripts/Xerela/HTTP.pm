...
sub new
{
	...
	# The HTTP agent will need to make heavy use of the Xerela::HTTPClient class to provide the functionality
	# needed to get/retrieve data via HTTP/HTTPS.  Default the timeout to 30 seconds.
	$this->{client} = Xerela::HTTPClient->new();
	$this->{client}->timeout(30);
	$this->{client}->protocols_allowed( [ 'http', 'https' ] );

	# Turn $this into a Xerela::HTTP object
...

{
	...
	sub new
	{
		...
		# Turn $this into a Xerela::HTTPClient object
		bless( $this, $class_name );
...
Xerela::HTTP - Agent/client for performing various HTTP/HTTPS requests.
...
C<Xerela::HTTP> serves an agent/client for performing various HTTP/HTTPS requests.  It utilizes much of the C<LWP> modules
...
requirement of C<Xerela::HTTP> explicitly, but is implicit because the underlying functionality provided by the LWP and
...
Creates a new instance of the C<Xerela::HTTP> module with a default timeout value of 30 seconds.  This value can be altered
by a call to set_timeout($timeout).

After creating a new instance of the C<Xerela::HTTP> module, it is assumed that the
...
Sends a HTTP C<GET> request to a URL that is relative the the hostname/IP address that this C<Xerela::HTTP>
object is currently connected to; the results from the request will be retrieved.

$relative_url -	A URL that is relative to the hostname/IP address that this C<Xerela::HTTP object> is already connected to.
...
Sends a HTTP C<POST> request to a URL that is relative the the hostname/IP address that this C<Xerela::HTTP>
...
Retrieves the IP address that this C<Xerela::HTTP> object is connected to.

=item C<get_port()>

Retrieves the port that this C<Xerela::HTTP> object is connected over.

=item C<get_protocol_name()>

Retrieves the name of the protocol used by this C<Xerela::HTTP> object for connecting to a server.  This will either be
"HTTP" or "HTTPS" but will be case-insensitive.

=item C<get_timeout()>

Retrieves the time, in seconds, that this C<Xerela::HTTP> object will wait before timing out during a connection or specific
request.

=item C<set_timeout($timeout)>

Specifies the time, in seconds, that this C<Xerela::HTTP> object will wait before timing out during a connection or specific
request.

=back

=head1 Xerela::HTTPClient Inner Class

=head2 NAME

C<Xerela::HTTPClient> - Inner-class of the C<Xerela::HTTP> module to handle username/password credential authentication.

=head2 SYNOPSIS

=head2 DESCRIPTION

The C<Xerela::HTTPClient> is an inner-class of the C<Xerela::HTTP> module that handles username/password credential
...
Creates a new instance of the C<Xerela::HTTPClient> class, which is just an implementation of the C<LWP::UserAgent> class.
