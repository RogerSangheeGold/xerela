package Xerela::Adapters::Cisco::Three005::Parsers;

use strict;
use warnings;
use Xerela::Addressing::Subnet;
use Xerela::Adapters::Utils qw(mask_to_bits seconds_since_epoch get_mask get_port_number trim strip_mac get_interface_type get_crep mask_to_bits);
...
sub parse_interfaces
{
	...
	if ($in->{interfaces} =~ /^\s*-+\s*$(.*)^\s*-+\s*$/ms)
	{
		...
		while($var =~ /^\s*(\S+)\s*\|\s*(\S+)\s*\|\s*($CIPM_RE)\/($CIPM_RE)\s*\|\s*($MAC)\s*/mogi)
		{
			...
			my $subnet = new Xerela::Addressing::Subnet( $3, mask_to_bits($4) );
...
Xerela::Adapters::Cisco::Three005::Parsers

=head1 SYNOPSIS

	use Xerela::Adapters::Cisco::Three005::Parsers qw(parse_routing create_config parse_local_accounts parse_chassis parse_filters parse_snmp parse_system parse_interfaces parse_static_routes parse_vlans parse_stp);
	parse_filters( $cliResponses, $xmlPrinter);	

=head1 DESCRIPTION

Module with many static methods that take in a hash of
device responses and print out Xerela model elements.
...
Uses the device configs to put together a Xerela configurationRepository element.
...
Parses out some top level core attributes of the Xerela model XerelaElementDocument
