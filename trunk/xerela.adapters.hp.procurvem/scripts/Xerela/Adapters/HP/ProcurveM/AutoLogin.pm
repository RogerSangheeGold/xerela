package Xerela::Adapters::HP::ProcurveM::AutoLogin;
...
sub _initial_connection
{
	...
	push( @responses, Xerela::Response->new( 'TELNET - OPERATOR MODE', \&_go_enable ) );
	push( @responses, Xerela::Response->new( 'TELNET - MANAGER MODE', \&_go_cli_mode ) );

	#push(@responses, Xerela::Response->new( 'TELNET - OPERATOR MODE', \&_send_enable ) );
	#push(@responses, Xerela::Response->new('item and press <Enter>\.\s*$', \&_get_prompt));
...
sub _send_password
{
	...
	# Specify the responses to check for
	my @responses = ();
	push( @responses, Xerela::Response->new( '[Ii]ncorrect|[Ii]nvalid', undef, $INVALID_CREDENTIALS ) );
	push( @responses, Xerela::Response->new( 'TELNET - OPERATOR MODE', \&_go_enable ) );
	push( @responses, Xerela::Response->new( 'TELNET - MANAGER MODE', \&_go_cli_mode) );
...
sub _go_cli_mode 
{
	...
	my $credentials  = shift;
	my @responses = ();
	push( @responses, Xerela::Response->new( 'Select VLAN',        	       \&_send_enter) );
	push( @responses, Xerela::Response->new( 'Diagnostics Menu',         	\&_send_4) );
	push( @responses, Xerela::Response->new( 'Main Menu',         		\&_send_5) );
	push( @responses, Xerela::Response->new( '\S*:',         		\&_calculate_prompt_regex) );
	my $response = $cli_protocol->wait_for_responses( \@responses );
	if ($response)
	{
		my $next_interaction = $response->get_next_interaction();
...
sub _go_enable
{
	...
	if ( $credentials->{enablePassword} )
	{
		# Specify the responses to check for
		my @responses = ();
		push( @responses, Xerela::Response->new( 'sername:|ogin:', \&_send_enable ) );
...
sub _send_enable_password
{
	...
	my @responses = ();
	push( @responses, Xerela::Response->new( '[Ii]ncorrect|[Ii]nvalid', undef, $INVALID_CREDENTIALS ) );
	push( @responses, Xerela::Response->new( 'TELNET - MANAGER MODE', \&_get_prompt ) );
...
Xerela::Adapters::HP::ProcurveM::AutoLogin - An example autologin module

=head1 SYNOPSIS

    use Xerela::Adapters::HP::ProcurveM;
	my $cli_protocol = Xerela::CLIProtocolFactory::create( $connection_path );
	my $prompt_regex = Xerela::Adapters::HP::ProcurveM::execute( $cli_protocol, $connection_path );
