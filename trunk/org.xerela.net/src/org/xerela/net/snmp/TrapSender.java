...
public final class TrapSender
{
    ...
    private static final String XERELA_EID = ".1.3.6.1.4.1.29510";
    private static final String XERELA_TRAP_OBJECTS = XERELA_EID + ".2";
    private static final String XERELA_TRAP = XERELA_EID + ".1";
    private static final int CHAANGE_TRAP_TYPE = 1;
    private static final OID CHANGE_TRAP_OID = new OID(XERELA_TRAP + "." + CHAANGE_TRAP_TYPE);
    private static final int DEVICE_ADD_TYPE = 2;
    private static final OID DEVICE_ADD_TRAP_OID = new OID(XERELA_TRAP + "." + DEVICE_ADD_TYPE);
    private static final int DEVICE_DELETE_TYPE = 3;
    private static final OID DEVICE_DELETE_TRAP_OID = new OID(XERELA_TRAP + "." + DEVICE_DELETE_TYPE);
    private static final int FAILED_OPERATION_TYPE = 4;
    private static final OID FAILED_OPERATION_TRAP_OID = new OID(XERELA_TRAP + "." + FAILED_OPERATION_TYPE);
     ...
     * @param managedNetwork the Xerela managedNetwork that the device lives in
     ...
     * @param adapterId the full Adapter ID associated with this device.  e.g. "Xerela::Adapters::Cisco::IOS"
    ...
    /**
     * Sends a Xerela enterprise trap and puts the properties inside the arguments as varbindings on the trap.
     * 
    ...
    public void sendTrap(OID trap, int specificType, List<VariableBinding> arguments)
    {
        if (hasTrapReceivers())
        {
            ...
            pdu.add(new VariableBinding(SnmpConstants.snmpTrapEnterprise, new OID(XERELA_EID)));
    ...
    private VariableBinding getVarBind(TrapObjects objectType, String value)
    {
        ...
        {
        case deviceHostname:
            vb.setOid(new OID(XERELA_TRAP_OBJECTS + ".1"));
            vb.setVariable(new OctetString(varValue));
            break;
        case deviceIpAddress:
            vb.setOid(new OID(XERELA_TRAP_OBJECTS + ".2"));
            vb.setVariable(new IpAddress(varValue));
            break;
        case managedNetwork:
            vb.setOid(new OID(XERELA_TRAP_OBJECTS + ".3"));
            vb.setVariable(new OctetString(varValue));
            break;
        case configurationName:
            vb.setOid(new OID(XERELA_TRAP_OBJECTS + ".4"));
            vb.setVariable(new OctetString(varValue));
            break;
        case adapterName:
            vb.setOid(new OID(XERELA_TRAP_OBJECTS + ".5"));
            vb.setVariable(new OctetString(varValue));
            break;
        case osType:
            vb.setOid(new OID(XERELA_TRAP_OBJECTS + ".6"));
            vb.setVariable(new OctetString(varValue));
            break;
        case operationName:
            vb.setOid(new OID(XERELA_TRAP_OBJECTS + ".7"));
            vb.setVariable(new OctetString(varValue));
            break;
        case messageDetail:
            vb.setOid(new OID(XERELA_TRAP_OBJECTS + ".8"));
            vb.setVariable(new OctetString(varValue));
            break;
        default:
            break;
