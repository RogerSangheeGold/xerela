package Xerela::Adapters::Alcatel::Omniswitch::AutoLogin;
...
sub _initial_connection
{
	...
	my @responses = ();
	push(@responses, Xerela::Response->new('sername:|(?<!Last)\s+login\s*:', \&_send_username));
...
sub _send_username
{
	...
	my @responses = ();
	push(@responses, Xerela::Response->new('[Pp]assword:', \&_send_password));
	push(@responses, Xerela::Response->new('incorrect|Authentication failure', undef, $INVALID_CREDENTIALS));
...
sub _send_password
{
	...
	# Specify the responses to check for
	my @responses = ();
	push(@responses, Xerela::Response->new('incorrect|login invalid|Authentication failure', undef, $INVALID_CREDENTIALS));
	push(@responses, Xerela::Response->new('(#|\$|>|%)\s*$', \&_get_prompt));
