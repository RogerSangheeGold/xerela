...
Creates the client.
  ...
  password:  The Xerela server password
  host:      The Xerela server host and port.  (Defaults to 'localhost:8080')
...
sub port
{
    ...
    else
    {
        ...
        my $token = $ENV{'XERELA_AUTHENTICATION'};
        if ($token)
        {
            $primary_url = $token;
        }
...
Please report any bugs or feature requests through the xerela bugzilla
web interface at L<http://bugs.xerela.org/>.
