package Xerela::Adapters::Juniper::JUNOS;
...
use Xerela::Adapters::Juniper::JUNOS::Parsers qw(parse_routing parse_interfaces create_config parse_local_accounts parse_chassis parse_filters parse_snmp parse_system);
...
sub commands
{
	...
	my $result = Xerela::Adapters::GenericAdapter::execute_cli_commands('JUNOS', $cli_protocol, $commands, $prompt_regex.'|([^\s#]+#|\$|^[^\s>]+>)\s*$');
	disconnect($cli_protocol);
	return $result;
}
...
sub _connect
{
	...
	my $device_prompt_regex = Xerela::Adapters::Juniper::JUNOS::AutoLogin::execute( $cli_protocol, $connection_path );
...
Xerela::Adapters::Juniper::JUNOS - Adapter for performing various operations against Juniper JUNOS devices.

=head1 SYNOPSIS

    use Xerela::Adapters::Juniper::JUNOS;
	Xerela::Adapters::Juniper::JUNOS::backup( $backup_document );
