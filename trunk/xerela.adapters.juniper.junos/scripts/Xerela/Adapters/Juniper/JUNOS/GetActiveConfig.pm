package Xerela::Adapters::Juniper::JUNOS::GetActiveConfig;
...
Xerela::Adapters::Juniper::JUNOS::GetActiveConfig - Retrieves the active configuration from an JUNOS-based device.

=head1 SYNOPSIS

    use Xerela::Adapters::Juniper::JUNOS::GetActiveConfig qw(get_active_config);
	my $active_config = get_active_config($cli_protocol_obj);
	my $active_config = get_active_config($cli_protocol_obj, $connection_path);

=head1 DESCRIPTION

C<Xerela::Adapters::Juniper::JUNOS::GetActiveConfig> allows for the retrieve of the active configuration
from an JUNOS-based device by using either a C<Xerela::CLIProtocol> object to retrieve the active configuration
via the command line using the "file show /config/juniper.conf.gz" command, or by using a C<Xerela::ConnectionPath>
object to retrieve the active config via a file transfer protocol client/agent. The only file transfer protocol currently
supported is SCP.

The only subroutine a user of C<Xerela::Adapters::Juniper::JUNOS::GetActiveConfig> should be concerned with is 
...
Input:		$cli_protocol_obj -	A valid C<Xerela::CLIProtocol> object that is already connected to a JUNOS-based device.
...
command via a C<Xerela::CLIProtocol> object that has been previously connected to a JUNOS-based device.
...
valid C<Xerela::ConnectionPath> object containing protocol information representing SCP was specified.
