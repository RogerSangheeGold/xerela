package Xerela::Adapters::Juniper::JUNOS::Parsers;

use strict;
use warnings;
use Xerela::Adapters::Utils
...
sub parse_telemetry_interfaces
{
	...
	# creates the Xerela interfaces model by reading the Juniper interface-information
...
sub _normalize_ospf_type
{
	...
	# follow the Xerela network type enum
...
sub _process_mac
{
	...
	# to comply with the Xerela model
	my $twig = shift;
	if ($twig)
	{
		my $mac = $twig->text();
...
sub _is_physical
{
	...
	# even though Juniper may call their loopback a physical interface, xerela doesn't.
	my $name = shift;
...
Xerela::Adapters::Juniper::JUNOS::Parsers

=head1 SYNOPSIS

	use Xerela::Adapters::Juniper::JUNOS::Parsers;
	parse_filters( $xmlPrinter, $cliResponses );	

=head1 DESCRIPTION

Module with many static methods that take in a hash of
JUNOS device responses and print out Xerela model elements.
...
Uses the Juniper JUNOS active and candidate configs to put together a Xerela
configurationRepository element.

=item C<parse_local_accounts>

Uses the Juniper JUNOS configuration to put together the Xerela
