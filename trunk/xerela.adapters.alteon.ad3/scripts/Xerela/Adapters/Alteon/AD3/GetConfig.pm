package Xerela::Adapters::Alteon::AD3::GetConfig;
...
sub _get_config_tftp
{
	...
	my @responses = ();
	push(@responses, Xerela::Response->new('Error|nknown command', undef, $TFTP_ERROR));
	push(@responses, Xerela::Response->new('Enter name of file on TFTP server:', \&_specify_target_file));
...
sub _specify_tftp_address
{
	...
	# Specify the responses to check for
	my @responses = ();
	push(@responses, Xerela::Response->new('Error|nknown command|Enter hostname or IP address of TFTP server:', undef, $TFTP_ERROR));
	push(@responses, Xerela::Response->new('Enter name of file on TFTP server:', \&_specify_target_file));
	push(@responses, Xerela::Response->new('successfully tftp\'d', \&_finish));	
...
sub _specify_target_file()
{
	...
	# Specify the responses to check for
	my @responses = ();
	push(@responses, Xerela::Response->new('Error|nknown command|Enter hostname or IP address of TFTP server:|Enter name of file on TFTP server:', undef, $TFTP_ERROR));
	push(@responses, Xerela::Response->new('successfully tftp\'d', \&_finish));	
