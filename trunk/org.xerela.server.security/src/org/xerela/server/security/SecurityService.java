...
import org.xerela.zap.security.IUserSessionListener;
import org.xerela.zap.security.PermissionTracker;
...
public class SecurityService implements ISecurityServiceEx
{
     ...
     */
    private static final String ORG_XERELA_ACCESS_ALL = "org.xerela.access.all"; //$NON-NLS-1$
    ...
    public ZPrincipal getZPrincipal(String username)
    {
        synchronized (principalMap)
        {
            ...
            try
            {
                ...
                if (principal != null)
                {
                    // Expand ORG_XERELA_ACCESS_ALL permission into all available permissions.
                    if (principal.getRole().hasPermission(ORG_XERELA_ACCESS_ALL))
                    {
