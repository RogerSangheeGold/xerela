package Xerela::Adapters::ThreeCom::Switch4400::Parsers;

use strict;
use warnings;
use Xerela::Addressing::Subnet;
use Xerela::Adapters::Utils qw(mask_to_bits seconds_since_epoch get_mask get_port_number trim get_interface_type get_crep);
	...
	while ( $in->{eth_summary} =~ /^(\d+:\d+)\s+\S+\s+(\S[^\)]+\))\s+/mig )
	{
		...
		if ( !$done_ifs->{$if_number} )
		{
			...
			if ( $ports_ips->{$sub_index} )
			{
				...
				my $subnet = new Xerela::Addressing::Subnet( $ports_ips->{$sub_index}->{ipAddress}, $ports_ips->{$sub_index}->{mask} );
