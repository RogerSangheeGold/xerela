package Xerela::Adapters::Cisco::VxWorks::Parsers;

use strict;
use warnings;
use Xerela::Addressing::Subnet;
use Xerela::Adapters::Utils qw(mask_to_bits seconds_since_epoch get_mask get_port_number trim get_interface_type getUnitFreeNumber get_crep);
	...
	while ( $if_blob =~ /<[^>]+><a\s+href="([^"]+)">([^<]+)(?:<[^>]+>){1,}([^<]+)(?:<[^>]+>){1,}([\d\.]+)(?:<[^>]+>){1,}($cipm)(?:<[^>]+>){1,}($mac1)(?:<[^>]+>){1,}/mig )
	{
		...
		if ( $if_link =~ /ifIndex=(\d+)/i )
		{
			...
			if ( $in->{'ifi'.$if_id} =~ /<[^>]+>Current IP Address:<[^>]+>\s+(?:<[^>]+>{1,})($cipm)/mi )
			{
				...
				my $subnet = new Xerela::Addressing::Subnet( $ip, $mask );
