package Xerela::Adapters::Aruba::ArubaOS::AutoLogin;
...
sub _initial_connection
{
	...
	my @responses = ();
	push( @responses, Xerela::Response->new('[Uu]ser:', \&_send_username) );
...
sub _send_username
{
	...
	my @responses = ();
	push(@responses, Xerela::Response->new('[Pp]assword:', \&_send_password));
	push(@responses, Xerela::Response->new('incorrect|invalid|WARNING|User:', undef, $INVALID_CREDENTIALS));
...
sub _send_enable
{
	...
	{    
		# Specify the responses to check for
		my @responses = ();
		push( @responses, Xerela::Response->new( 'incorrect|invalid|WARNING', undef, $INVALID_CREDENTIALS ) );
...
Xerela::Adapters::Aruba::ArubaOS::AutoLogin - An example autologin module

=head1 SYNOPSIS

    use Xerela::Adapters::Aruba::ArubaOS;
	my $cli_protocol = Xerela::CLIProtocolFactory::create( $connection_path );
	my $prompt_regex = Xerela::Adapters::Aruba::ArubaOS::execute( $cli_protocol, $connection_path );
