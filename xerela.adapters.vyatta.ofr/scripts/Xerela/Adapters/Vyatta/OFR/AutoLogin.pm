package Xerela::Adapters::Vyatta::OFR::AutoLogin;
...
sub _initial_connection
{
	...
	my @responses = ();
	push( @responses, Xerela::Response->new( '(?<!Last)\s+login:\s*$', \&_send_username ) );
	push( @responses, Xerela::Response->new( 'assword:', \&_send_password ) );
	push( @responses, Xerela::Response->new( '.+~#\s+$',  \&_enter_xorpsh ) );
	push( @responses, Xerela::Response->new( '.+>',      \&_calculate_prompt_regex ) );
...
sub _send_password
{
	...
	# Specify the responses to check for
	my @responses = ();
	push( @responses, Xerela::Response->new( 'ogin incorrect', undef, $INVALID_CREDENTIALS ) );
	push( @responses, Xerela::Response->new( '.+~#\s+$', \&_enter_xorpsh ) );
	push( @responses, Xerela::Response->new( '.+>', \&_calculate_prompt_regex ) );
...
sub _enter_xorpsh
{
	...
	# Specify the responses to check for
	my @responses = ();
	push( @responses, Xerela::Response->new( 'command not found', undef, $INSUFFICIENT_PRIVILEGE ) );
	push( @responses, Xerela::Response->new( '.+>', \&_calculate_prompt_regex ) );
...
Xerela::Adapters::Vyatta::OFR::AutoLogin - Automates the connection and authentication with a Vyatta Open Flexible Router (OFR) device.

=head1 SYNOPSIS

    use Xerela::Adapters::Vyatta::OFR::AutoLogin;
	my $cli_protocol = Xerela::CLIProtocolFactory::create( $connection_path );
	my $prompt_regex = Xerela::Adapters::Vyatta::OFR::AutoLogin::execute( $cli_protocol, $connection_path );
...
of this auto-login module assume that C<Xerela::CLIProtocol> object is interfacing with the XORP shell on the OFR.
