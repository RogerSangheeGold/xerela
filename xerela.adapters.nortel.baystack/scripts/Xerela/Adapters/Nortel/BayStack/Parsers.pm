package Xerela::Adapters::Nortel::BayStack::Parsers;

use strict;
use warnings;
use Xerela::Addressing::Subnet;
use Xerela::Adapters::Utils qw(get_mask get_port_number trim get_interface_type);
use Xerela::Logger;
		...
		foreach my $interface ( @{ $ints->{interface} } )
		{
			if ( $interface->{name} =~ /\s+$port$/ )
			{
				...
				if ( $interface->{interfaceIp}->{ipConfiguration} )
				{
					...
					{
						$foundSubnets = 1;
						my $subnet = new Xerela::Addressing::Subnet( $ipAddress, $mask );
