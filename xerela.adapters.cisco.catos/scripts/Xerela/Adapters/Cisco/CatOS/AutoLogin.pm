package Xerela::Adapters::Cisco::CatOS::AutoLogin;
...
sub _send_username
{
	...
	my @responses = ();
	push( @responses, Xerela::Response->new( '[Pp]assword:', \&_send_password ) );
	push( @responses, Xerela::Response->new( 'incorrect|Username:', undef, $INVALID_CREDENTIALS ) );
...
sub _send_password
{
	...
	# Specify the responses to check for
	my @responses = ();
	push( @responses, Xerela::Response->new( 'sername:|ogin:|authentication failed', undef, $INVALID_CREDENTIALS ) );
	push( @responses, Xerela::Response->new( 'incorrect|invalid|[Pp]assword:', undef, $INVALID_CREDENTIALS ) );
	push( @responses, Xerela::Response->new( '\(enable\)\s*$', \&_get_prompt ) );
	push( @responses, Xerela::Response->new( '\n\S+\s$', \&_send_enable ) );
...
sub _send_enable
{
	...
	{
		my @responses = ();    
		push( @responses, Xerela::Response->new( 'incorrect|invalid|Sorry', undef, $INVALID_CREDENTIALS ) );
...
sub _send_enable_password
{
	...
	# Specify the responses to check for
	my @responses = ();
	push( @responses, Xerela::Response->new( 'incorrect|invalid|Sorry|[Pp]assword:', undef, $INVALID_CREDENTIALS ) );
	push( @responses, Xerela::Response->new( '(?:\(enable\)\s*|\#\s*)$', \&_get_prompt ) );
...
Xerela::Adapters::Cisco::CatOS::AutoLogin - Automates the connection and authentication with a Cisco CatOS device.

=head1 SYNOPSIS

    use Xerela::Adapters::Cisco::CatOS::AutoLogin;
	my $cli_protocol = Xerela::CLIProtocolFactory::create( $connection_path );
	my $prompt_regex = Xerela::Adapters::Cisco::CatOS::AutoLogin::execute( $cli_protocol, $connection_path );
