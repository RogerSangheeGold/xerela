package Xerela::Adapters::Nortel::Passport::Restore;
...
sub restore_via_tftp
{
	...
	push( @responses, Xerela::Response->new( 'already existing, overwrite \(y\/n\)', \&_confirm ) );
...
sub _confirm
{
	...
	my @responses = ();
	push( @responses, Xerela::Response->new( 'ERROR', undef, $TFTP_ERROR ) );
	push( @responses, Xerela::Response->new( 'already existing, overwrite \(y\/n\)', undef, $TFTP_ERROR ) );
	push( @responses, Xerela::Response->new( $promptRegex, \&_finish ) );
