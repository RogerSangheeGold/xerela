package Xerela::Adapters::HP::ProCurve::Restore;
...
use Xerela::Adapters::HP::ProCurve::Disconnect
	qw(disconnect);
use Xerela::Logger;
...
sub _connect
{
	...
	my $device_prompt_regex = Xerela::Adapters::HP::ProCurve::AutoLogin::execute( $cli_protocol, $connection_path );
...
sub restore_via_tftp
{
	...
	my @responses = ();
	push(@responses, Xerela::Response->new('[Ee]rror|[Ff]ail|Peer unreachable', undef, $TFTP_ERROR));
	push(@responses, Xerela::Response->new('do you want to continue', \&_confirm));
	push(@responses, Xerela::Response->new('Rebooting switch', \&_finish));
