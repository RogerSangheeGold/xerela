package Xerela::Adapters::HP::ProCurve::GetStartupConfig;
...
Xerela::Adapters::HP::ProCurve::GetStartupConfig - Retrieves the startup configuration from a ProCurve device.

=head1 SYNOPSIS

    use Xerela::Adapters::HP::ProCurve::GetStartupConfig qw(get_startup_config);
	my $startup_config = get_startup_config($cli_protocol);
	my $startup_config = get_startup_config($cli_protocol, $connection_path);

=head1 DESCRIPTION

C<Xerela::Adapters::HP::ProCurve::GetStartupConfig> allows for the retrieval of the startup configuration
from a ProCurve device by using either a C<Xerela::CLIProtocol> object to retrieve the startup configuration
via the command line using the "show config" command, or by using a C<Xerela::ConnectionPath> to retrieve
the startup config via a file transfer protocol client/agent. The only file transfer protocol that is supported
currently is TFTP.

The only subroutine a user of C<Xerela::Adapters::HP::ProCurve::GetStartupConfig> should be concerned with is 
