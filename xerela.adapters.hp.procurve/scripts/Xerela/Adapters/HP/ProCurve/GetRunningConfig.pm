package Xerela::Adapters::HP::ProCurve::GetRunningConfig;
...
sub _get_running_config_tftp
{
	...
	push(@responses, Xerela::Response->new($bad_command_regex, undef, $TFTP_ERROR));
	push(@responses, Xerela::Response->new('Incomplete input: -EOI-', undef, $TFTP_ERROR));
	push(@responses, Xerela::Response->new('Peer unreachable.', undef, $TFTP_ERROR));
...
Xerela::Adapters::HP::ProCurve::GetRunningConfig - Retrieves the running configuration from a ProCurve device.

=head1 SYNOPSIS

    use Xerela::Adapters::HP::ProCurve::GetRunningConfig qw(get_running_config);
	my $running_config = get_running_config($cli_protocol);
	my $running_config = get_running_config($cli_protocol, $connection_path);

=head1 DESCRIPTION

C<Xerela::Adapters::HP::ProCurve::GetRunningConfig> allows for the retrieval of the running configuration
from a ProCurve device by using either a C<Xerela::CLIProtocol> object to retrieve the running configuration
via the command line using the "show running-config" command, or by using a C<Xerela::ConnectionPath> to retrieve
the running config via a file transfer protocol client/agent. The file transfer protocols that is supported
currently is TFTP.

The only subroutine a user of C<Xerela::Adapters::HP::ProCurve::GetRunningConfig> should be concerned with is 
...
Input:		$cli_protocol -	A valid C<Xerela::CLIProtocol> object that is already connected to a ProCurve device.
...
command via a C<Xerela::CLIProtocol> object that has been previously connected to a ProCurve device.

This subroutine will be called by C<get_running_config($cli_protocol, $connection_path)> if no valid
C<Xerela::ConnectionPath> object was specified.
...
command via a C<Xerela::CLIProtocol> object that has already been connected to a ProCurve device.  This method
utilizes an array C<Xerela::Response> objects to handle different responses from the device.

This subroutine will be called by C<get_running_config($cli_protocol, $connection_path)> if a
valid C<Xerela::ConnectionPath> object containing information representing TFTP was specified.
...
it to the recording of the C<Xerela::CLIProtocol> object as a new interaction with the "cliCommand" element named
"show running-config".
