package Xerela::Adapters::Adtran::NetVanta::AutoLogin;
...
sub _send_password
{
	...
	# Specify the responses to check for
	my @responses = ();
	push(@responses, Xerela::Response->new('Login Failure', undef, $INVALID_CREDENTIALS));
	push( @responses, Xerela::Response->new( '\S+>',  		\&_send_enable ) );
	push(@responses, Xerela::Response->new('(#|\$)\s*$', \&_get_prompt));
...
sub _send_enable_password
{
	...
	# Specify the responses to check for
	my @responses = ();
	push( @responses, Xerela::Response->new( 'ncorrect password',		undef, $INVALID_CREDENTIALS ) );
	push( @responses, Xerela::Response->new( '\S+#\s*', 			\&_get_prompt ) );
