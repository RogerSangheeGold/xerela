package Xerela::Adapters::Juniper::ScreenOS;
...
use Xerela::Adapters::Juniper::ScreenOS::Restore;
...
sub commands
{
	...
	$cli_protocol->send_and_wait_for( 'set console page 0', $prompt_regex );
	my $result = Xerela::Adapters::GenericAdapter::execute_cli_commands( 'ScreenOS', $cli_protocol, $commands, $prompt_regex.'|(#|\$|>)\s*$' );
	disconnect($cli_protocol);
	return $result;
}
...
sub _connect
{
	...
	my $device_prompt_regex = Xerela::Adapters::Juniper::ScreenOS::AutoLogin::execute( $cli_protocol, $connection_path );
...
Xerela::Adapters::Juniper::ScreenOS - Example adapter for performing various operations against a particular family of devices.

=head1 SYNOPSIS

    use Xerela::Adapters::Juniper::ScreenOS;
	Xerela::Adapters::Juniper::ScreenOS::backup( $backup_document );
